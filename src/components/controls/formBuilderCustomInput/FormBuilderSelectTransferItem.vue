  ...
  <div class="form-builder-select-transfer-item">
    ...
    >
        ...
        <q-tr v-if="!readonly">
          <q-td>
            <div class="full-width">
              <q-select v-model="transferItemType"
          ...
          <q-td>
            <div class="full-width">
              <div v-if="transferItemType === 'App\\Models\\Product'" class="col-md-3">
                <select-product v-model:value="selectedProduct" :emit-value="false" :map-options="false" label="" />
              </div>
              <div v-if="transferItemType === 'App\\Models\\ProductPart'" class="col-md-3">
                <select-product-part v-model:value="selectedProductPart" :emit-value="false" :map-options="false" label="" />
              </div>
              <div v-if="transferItemType === 'App\\Models\\RawMaterial'" class="col-md-3">
                <select-raw-material v-model:value="selectedRawMaterial" :emit-value="false" :map-options="false" label="" />
              </div>
            </div>
          </q-td>
          <q-td>
            <div class="full-width">
              <q-input v-model="quantity" :disable="!selectedTransferId" :loading="loading" type="number" />
            </div>
          </q-td>
          <q-td>
            <div class="full-width">
              <q-input v-model="notes" :disable="!selectedTransferId" :loading="loading" />
            </div>
          </q-td>
          <q-td>
            <div class="full-width flex justify-center">
              <q-btn color="primary" icon="add" :disable="!isValidForm" class="q-mt-md" @click="addItem" />
            </div>
          </q-td>
        </q-tr>
      </template>
      <template #body-cell-action="tblProps">
        <q-td :props="tblProps">
          <q-btn icon="close" flat @click="removeItem(tblProps)" />
        </q-td>
      </template>
    </q-table>
  </div>
